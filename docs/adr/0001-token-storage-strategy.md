...
**Status:** Resolved in Milestone 12 (Production & Deployment) — see "Resolution" below
**Date:** Milestone 2.1 (Core Authentication); resolved Milestone 12
...
## Resolution (Milestone 12)

Implemented as described above, with one addition not anticipated in the
original "What Would Need to Change" analysis: a **double-submit CSRF
cookie**. The CSRF protection note above was correct that the refresh
endpoint needed it, but the mechanism chosen was a self-contained
comparison rather than Django's own CSRF middleware — `rest_framework`'s
`APIView` unconditionally marks every view `csrf_exempt`, so
`CsrfViewMiddleware` never runs for any DRF endpoint regardless of
authentication class, and Django's CSRF token masking internals aren't
public API to build against. Instead, a second, non-httpOnly cookie
(`mm_csrf`) carries a random token that only same-origin JavaScript can
read and echo back as an `X-CSRF-Token` header; a cross-site attacker's
browser sends the refresh cookie automatically but can't read the CSRF
cookie's value to forge a matching header.

What actually shipped:

- `apps/accounts/cookies.py` — `attach_refresh_cookie`, `clear_refresh_cookie`, `verify_csrf`.
- `LoginView`, `RegisterView`, `GoogleLoginView`, `TokenRefreshThrottledView`,
  `LogoutView` (`apps/accounts/views.py`) — refresh token moved out of the
  JSON body into the httpOnly cookie; refresh/logout read the cookie and
  require the CSRF header.
- `CORS_ALLOW_CREDENTIALS = True`, `AUTH_COOKIE_*` settings
  (`config/settings/base.py`, hardened further in `production.py`).
- Frontend: `api/client.ts` (`withCredentials: true`, cookie-based silent
  refresh), `api/cookies.ts` (reads the CSRF cookie), `features/auth/store.ts`
  (refresh token removed entirely — the store is no longer persisted to
  `localStorage`), `features/auth/bootstrap.ts` (restores a session on app
  load via the httpOnly cookie, since nothing about the session survives a
  page reload in JS-reachable state anymore).
- Test coverage: `apps/accounts/tests/test_cookie_auth.py` (8 tests —
  cookie flags, refresh success/failure paths, CSRF enforcement, logout
  clearing both cookies) plus updated assertions in
  `test_registration_and_login.py` and `test_google_oauth.py`.

The XSS-exfiltration path this ADR flagged is closed: the refresh token
never touches `document.cookie`, `localStorage`, or any other
JavaScript-readable storage from this point forward.
