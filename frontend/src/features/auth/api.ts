...
/**
 * Logout. The refresh token is read server-side from the httpOnly cookie
 * (ADR 0001) — nothing to pass here except the CSRF header the backend
 * requires to guard against a cross-site request forgery of this endpoint
 * (apps/accounts/cookies.py `verify_csrf`).
 */
export async function logoutRequest(): Promise<void> {
  await apiClient.post('/auth/logout/', {}, { headers: getCsrfHeader() });
}
