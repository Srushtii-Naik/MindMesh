...
type SidebarTab = 'suggestions' | 'memory';
 ...
 * stub that occupied ROUTES.AI_CHAT since Milestone 3. Milestone 8 adds a
 * "Memory" tab alongside Suggestions so users can see, edit, and delete
 * what MindMesh remembers about them (PRD.md Section 13).
 */
  ...
  return (
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 md:grid-cols-[220px_1fr_260px]">
        ...
        <aside className="flex min-h-0 flex-col rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800">
          <div
            role="tablist"
            aria-label="AI companion sidebar"
            className="mb-3 flex gap-1 rounded-md bg-slate-100 p-1 text-sm dark:bg-slate-900"
          >
            <button
              type="button"
              role="tab"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {sidebarTab === 'suggestions' ? <SuggestionsPanel /> : <MemoryPanel />}
          </div>
        </aside>
      </div>
    </motion.div>
