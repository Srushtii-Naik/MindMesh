...
CELERY_BEAT_SCHEDULE = {
    ...
    'analytics-generate-weekly-reports': {
        'task': 'analytics.generate_weekly_reports',
        # ROADMAP.md Milestone 11: "Progress reports generated on a
        # sensible cadence (e.g., weekly)". Runs once a day; the service
        # layer is idempotent per (user, period_start, period_end) so a
        # daily check costs nothing extra and tolerates a missed beat tick
        # without ever skipping a user's weekly report.
...
# Required for the httpOnly-cookie refresh token (see AUTH_REFRESH_COOKIE_NAME
# below) to be sent by the browser on cross-origin requests from the Vercel
# frontend to the Railway backend. Safe to enable only because
# CORS_ALLOWED_ORIGINS remains a strict allow-list, never a wildcard —
# credentialed requests cannot use '*' per the CORS spec, so this pairing is
# the only way both requirements (credentials + strict origin) are satisfied.
CORS_ALLOW_CREDENTIALS = True

# --------------------------------------------------------------------------
# Auth cookie configuration (Milestone 12 — ADR 0001 resolution).
#
# Per ADR 0001 (docs/adr/0001-token-storage-strategy.md) and PROJECT_RULES.md
# Section 8, the JWT refresh token is delivered as an httpOnly cookie rather
# than in the JSON response body, removing it from JavaScript's reach (and
# therefore from XSS exfiltration). A companion, non-httpOnly "double
# submit" CSRF cookie (also below) protects the endpoints that read the
# refresh cookie (token refresh, logout) from cross-site request forgery.
# See apps/accounts/cookies.py for how these are set/verified.
# --------------------------------------------------------------------------

AUTH_REFRESH_COOKIE_NAME = env('AUTH_REFRESH_COOKIE_NAME', default='mm_refresh')
AUTH_CSRF_COOKIE_NAME = env('AUTH_CSRF_COOKIE_NAME', default='mm_csrf')
# False in development (plain HTTP over localhost); forced True in production.py.
AUTH_COOKIE_SECURE = env.bool('AUTH_COOKIE_SECURE', default=False)
# 'Lax' works for same-site local dev (localhost:5173 -> localhost:8000).
# Production.py defaults this to 'None' since Vercel and Railway are
# different origins, which requires Secure=True (enforced by the browser).
AUTH_COOKIE_SAMESITE = env('AUTH_COOKIE_SAMESITE', default='Lax')
# Left blank by default (host-only cookie). Set only if frontend/backend
# share a registrable parent domain (e.g. both under *.mindmesh.app).
...
# --------------------------------------------------------------------------
# Logging (Milestone 12 — Production & Deployment).
#
# Writes structured, single-line records to stdout rather than a file —
# Railway (and Docker generally) captures container stdout/stderr directly,
# so a file-based handler would be invisible to the platform's log
# aggregation and unnecessarily add local disk state to a stateless backend
# process (ARCHITECTURE.md Section 11 — "stateless backend processes").
#
# 'mindmesh.security' is the dedicated logger for security-sensitive
# actions (PROJECT_RULES.md Section 8 — "login, password change, OAuth
# linking, data export/delete ... logged for auditability"); it always
# logs at INFO so these records are never silenced by a WARNING-level root
...
LOGGING = {
    ...
    'loggers': {
        ...
        # Security-sensitive action audit trail — see docstring above.
        'mindmesh.security': {
            'handlers': ['console'],
            'level': 'INFO',
            'propagate': False,
...
# --------------------------------------------------------------------------
# Baseline security headers (ARCHITECTURE.md Section 10). Safe defaults in
# every environment; production.py adds the headers that only make sense
# under HTTPS (HSTS, secure cookies, SSL redirect).
# --------------------------------------------------------------------------
...
# Trusted origins for Django's own CSRF machinery (used by the session-
# authenticated admin site, not the JWT API — see apps/accounts/cookies.py
# for the API's own double-submit CSRF check). Empty by default in
# development (admin is same-origin at localhost:8000); production.py
