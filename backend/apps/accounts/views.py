...
# Dedicated logger for security-sensitive actions (PROJECT_RULES.md Section 8
# — "login, password change, OAuth linking ... logged for auditability"),
# configured in config/settings/base.py to always log at INFO regardless of
# the root log level.
security_logger = logging.getLogger('mindmesh.security')
...
class RegisterView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        response = Response(
            {'user': UserPublicSerializer(user).data, 'access': tokens['access']},
            status=status.HTTP_201_CREATED,
        )
        attach_refresh_cookie(response, tokens['refresh'])
        security_logger.info('user_registered user_id=%s email=%s', user.pk, user.email)
        return response
...
class LoginView(TokenObtainPairView):
    ...
    Per ADR 0001 / PROJECT_RULES.md Section 8, the refresh token issued by
    simple-jwt's base `post()` is moved out of the JSON body and into an
    httpOnly cookie before the response is returned — see
    apps.accounts.cookies.
    """
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            security_logger.info('user_login email=%s', email)
        return response
...
class TokenRefreshThrottledView(TokenRefreshView):
    ...
    Reads the refresh token from the httpOnly cookie (ADR 0001) rather than
    the request body, and requires the double-submit CSRF header to match
    the CSRF cookie (apps.accounts.cookies.verify_csrf) before honoring the
    request — the browser attaches the refresh cookie automatically, so
    without this check any site could trigger a token refresh/rotation for
    a logged-in user. Rate limiting (PROJECT_RULES.md Section 8) is
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if not refresh_token:
            return Response(
                {'detail': 'No refresh session found.', 'code': 'refresh_required'},
        ...
        # SIMPLE_JWT.ROTATE_REFRESH_TOKENS is True, so a new refresh token is
        # always present here; the `or refresh_token` fallback only guards
        # against that setting being turned off in a future change.
        attach_refresh_cookie(response, data.get('refresh') or refresh_token)
        return response
...
class LogoutView(APIView):
    ...
    Blacklists the refresh token (read from the httpOnly cookie, per ADR
    0001) so it can no longer be used to obtain new access tokens, and
    clears both auth cookies. Requires a valid access token — only an
    authenticated session can log itself out — and the double-submit CSRF
    header, for the same reason as token refresh (see
    ...
    def post(self, request: Request) -> Response:
        ...
        if not refresh_token:
            return Response(
                {'detail': 'No active session to log out.', 'code': 'refresh_required'},
        ...
        security_logger.info('user_logout user_id=%s', request.user.pk)
        return response
...
class GoogleLoginView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        attach_refresh_cookie(response, tokens['refresh'])
        security_logger.info('user_login_google user_id=%s email=%s', user.pk, user.email)
        return response
...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        security_logger.info('password_reset_completed uid=%s', validated['uid'])
        return Response(
...
class SessionRevokeView(APIView):
    ...
    def post(self, request: Request, session_id: int) -> Response:
        ...
        security_logger.info(
            'session_revoked user_id=%s session_id=%s', request.user.pk, session_id
        )
...
class SessionRevokeAllView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        security_logger.info('all_sessions_revoked user_id=%s', request.user.pk)
        return Response(status=status.HTTP_204_NO_CONTENT)
