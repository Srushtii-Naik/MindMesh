...
from apps.tasks.repositories import (
    ...
    count_tasks_completed_between,
    count_tasks_completed_on,
    count_tasks_created_between,
    count_tasks_due_on,
    count_tasks_overdue,
    create_category,
    create_subtask,
    create_task,
    daily_completed_counts_between,
    daily_created_counts_between,
    delete_category,
...
# --------------------------------------------------------------------------
# Productivity analytics & habit tracking (ROADMAP.md Milestone 11).
# Cross-domain entry points for apps.analytics — never expose the Task model
# itself, per ARCHITECTURE.md Section 3.
# --------------------------------------------------------------------------


def get_task_productivity_stats(user: User, start_date: date, end_date: date) -> dict:
    """Task creation/completion counts and daily breakdowns for
    apps.analytics' productivity analytics (ROADMAP.md Milestone 11)."""
    created = count_tasks_created_between(user, start_date, end_date)
    completed = count_tasks_completed_between(user, start_date, end_date)
    return {
        'tasks_created': created,
        'tasks_completed': completed,
        'completion_rate': round((completed / created) * 100, 1) if created else 0.0,
        'completed_by_day': daily_completed_counts_between(user, start_date, end_date),
        'created_by_day': daily_created_counts_between(user, start_date, end_date),
    }


def get_completed_dates_for_streak(user: User, start_date: date, end_date: date) -> set[date]:
    """Dates on which the user completed at least one task, for
    apps.analytics' habit-streak computation (ROADMAP.md Milestone 11)."""
    return set(daily_completed_counts_between(user, start_date, end_date).keys())
