...
def count_tasks_created_between(user: User, start_date: date, end_date: date) -> int:
    return Task.objects.filter(
        user=user, is_active=True, created_at__date__gte=start_date, created_at__date__lte=end_date
    ).count()


def count_tasks_completed_between(user: User, start_date: date, end_date: date) -> int:
    return Task.objects.filter(
        user=user,
        is_active=True,
        is_completed=True,
...
def daily_created_counts_between(user: User, start_date: date, end_date: date) -> dict[date, int]:
    """Per-day counts of tasks created in [start_date, end_date] (ROADMAP.md
    Milestone 11 — productivity analytics). Grouped in the database via
    TruncDate rather than one query per day."""
...
def daily_completed_counts_between(user: User, start_date: date, end_date: date) -> dict[date, int]:
    """Per-day counts of tasks completed in [start_date, end_date] (ROADMAP.md
    Milestone 11 — productivity analytics & habit streaks)."""
    from django.db.models import Count
    from django.db.models.functions import TruncDate
