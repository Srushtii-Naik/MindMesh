...
def get_event_count_for_range(user: User, start_date: date, end_date: date) -> int:
    """Cross-domain entry point for apps.analytics' productivity analytics
    (ROADMAP.md Milestone 11) — the count of events overlapping the given
    range, without exposing the Event model itself (ARCHITECTURE.md
    Section 3)."""
    range_start, _ = _day_bounds(start_date)
    _, range_end = _day_bounds(end_date)
    return list_events_overlapping_range(user, range_start, range_end).count()
